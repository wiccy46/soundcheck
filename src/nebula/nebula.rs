...
pub async fn active_channels(_base_url: &Url) -> Result<HashMap<String, i16>, String>{
    ...
    let preset = fetch_preset_id(&_base_url, &preset).await.map_err(|e| e.to_string())?;
    beam_groups_ids = preset.beam_group_ids;
    println!("Active preset name: {}", preset.name);

    let mut beam_instances_ids: Vec<String> = Vec::new();

    let beam_groups = fetch_beam_groups(&_base_url).await.map_err(|e| e.to_string())?;
    ...
    let beam_instances = fetch_beam_instances(&_base_url).await.map_err(|e| e.to_string())?;
    ...
    let beam_audio_inputs = fetch_beam_audio_inputs(&_base_url).await.map_err(|e| e.to_string())?;
    ...
    let audio_inputs = fetch_audio_inputs(&_base_url).await.map_err(|e| e.to_string())?;
    ...
    let audio_input_stream_mappings = fetch_audio_input_stream_mappings(&_base_url).await.map_err(|e| e.to_string())?;  
